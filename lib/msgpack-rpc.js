...
function MsgpackRPCStream(stream, handler) {
...
util.inherits(MsgpackRPCStream, events.EventEmitter);
...
exports.createClient = function (port, hostname, cb) {
  var s = new MsgpackRPCStream(new net.connect(port, hostname));
...
SessionPool.prototype.closeClients = function() {
  this.clients.forEach(function (c) { c.stream.end(); });
};
